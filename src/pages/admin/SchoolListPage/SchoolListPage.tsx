...
const getMeasurementPeriod = (item: SchoolListItem): string => {
  const currentYearEntry =
    item.supported_years.find((sy) => sy.year === CURRENT_YEAR) ??
...
export const SchoolListPage = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<
    "all" | "active" | "inactive"
  >("all");
  const [yearFilter, setYearFilter] = useState<number | "all" | "noSupport">(
    CURRENT_YEAR,
  );
  ...
  const sizeTypeToApiSizes = (sizeType: string) => {
    ...
    if (sizeType === "numeric_5")
      return [
        {
          size: "numeric",
          size_type: "numeric" as const,
          size_step: 5 as const,
        },
      ];
    if (sizeType === "numeric_3")
      return [
        {
          size: "numeric",
          size_type: "numeric" as const,
          size_step: 3 as const,
        },
      ];
    if (sizeType === "alpha")
      return [{ size: "alpha", size_type: "alpha" as const }];
    if (sizeType === "free")
      return [{ size: "free", size_type: "free" as const }];
    return undefined;
  };
  ...
  const handleAddProduct = async (data: ProductAddData) => {
    try {
      const created = await createProduct({
        ...
        gender: data.gender,
        is_repair:
          data.isRepairable === "yes"
            ? true
            : data.isRepairable === "no"
              ? false
              : undefined,
        is_repair_required:
          data.isRepairRequired === "required"
            ? true
            : data.isRepairRequired === "optional"
              ? false
              : undefined,
  ...
  const columns: Column<SchoolRow>[] = [
    ...
    { key: "type", header: "구분", width: "90px", align: "center" },
    {
      key: "supportYears",
      header: "지원년도",
      width: "120px",
    ...
    { key: "modifiedDate", header: "수정일", width: "100px", align: "center" },
    {
      key: "actions",
      header: "관리",
      width: "80px",
            ...
            <div className="flex items-center gap-4 flex-1 px-4 py-3 bg-white">
              ...
              <label className="flex items-center gap-1.5 text-14 text-gray-700 cursor-pointer">
                <input
                  type="radio"
                  name="yearFilter"
                  checked={yearFilter === "noSupport"}
                  onChange={() => setYearFilter("noSupport")}
                  className="w-4 h-4 accent-gray-500"
                />
                미지원
              </label>
      ...
      <ProductAddModal
        ...
        onRemoveSchool={(id) =>
          setSelectedSchoolsForProduct((prev) =>
            prev.filter((s) => s.schoolId !== id),
          )
        }
        onSchoolPriceChange={(id, price) =>
          setSelectedSchoolsForProduct((prev) =>
            prev.map((s) => (s.schoolId === id ? { ...s, price } : s)),
          )
        }
        zIndex={1100}
      />
