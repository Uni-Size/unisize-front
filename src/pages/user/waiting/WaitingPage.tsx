...
export default function WaitingPage() {
  ...
  const TableView = ({
    ...
    <div className="w-full bg-amber-50 p-4 rounded-lg">
      <h3 className="font-semibold mb-6">{season}</h3>
      ...
      <div className="">
        ...
        {tableData.map((row, idx) => (
          <div key={idx} className="grid grid-cols-3 py-2 text-center">
            ...
            <div>
              {row.count === 0 ? (
                <p className="text-xs text-amber-600 ">지원제외품목</p>
              ) : (
                row.count
              )}
            </div>
  ...
  return (
    <section className="p-4">
      ...
      <div className="text-center m-6">
        <p className="text-base font-medium">잠시만 기다려주세요.</p>
        <h2 className="text-2xl font-bold my-2">
          {studentData.school_name} {studentData.name}
        </h2>
        <p className="text-lg text-gray-700">교복 시착을 준비해드리겠습니다.</p>
      </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {itemData[activeTab].map((product, idx) => (
            <a
              ...
              className="block bg-gradient-to-r rounded-lg hover:shadow-md transition-shadow duration-200 p-2 bg-gray-50"
            >
